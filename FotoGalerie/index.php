...
class FotoGalerie extends Plugin {
  ...
  const FG_SINGLE_PREVIEW = "EinzelVorschauGalerie";
  ...
  private $lang_gallery_cms;
    ...
    ***************************************************************/
    function getInfo() {
        ...
        $info = array(
            // Plugin-Name
            "<b>".$this->lang_gallery_admin->getLanguageValue("config_fotogallery_plugin_name")."</b> \$Revision: 9 $",
            // CMS-Version
            "2.0",
            // Kurzbeschreibung
           ...
           "black-night",
            ...
            array('{FotoGalerie|...}' => $this->lang_gallery_admin->getLanguageValue("config_fotogallery_plugin_name"),
                  ...
                  '{FotoGalerie|'.self::FG_SINGLE_PREVIEW.',...}' => $this->lang_gallery_admin->getLanguageValue("config_fotogallery_plugin_single_preview")
                 )
            );
            return $info;        
    } // function getInfo
    ...
    *
    ***************************************************************/
    private function getFullGalerie($galleryname, $singlePreview = false) {
      global $CMS_CONF;
      global $specialchars;
      $dir = $this->PLUGIN_SELF_DIR;
      ...
      $group = "group-".md5($galleryname); 
      $i = 0;
      $result = "<div id=\"".$this->getIDName($galleryname)."\">";
      for ($i=0; $i<count($picarray); $i++) {
        $style = "";
        if($i==1 && $singlePreview === true)
          $result .= "<div class=\"FotoGalerieHidden\" style=\"width: 0; height: 0; overflow: hidden;\">";
      ...
      if (count($picarray) > 1 && $singlePreview === true)
        $result .= "</div>";
    ...
    private function getSpezialGalerie($typ,$galleryname) {
      ...
      }elseif ($typ == self::FG_SINGLE_PREVIEW) {
        $result = $this->getFullGalerie($galleryname,true);
      }
      return $result;
    } //getSpezialGalerie
